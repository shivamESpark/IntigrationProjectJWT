...
const dynamicComponent = (req, res, next)=>{
    const baseUrl = url.parse(req.url, true);
    let field = baseUrl.query.comp;


    qry = `select s.select_field as field_name, s.unique_name as unique_name, s.type as type, s.multipleValue as multi, o.option_key as option_key, o.option_value as option_value from option_master as o join select_master as s on s.id = o.sid where select_field = ?;`;
    ...
    con.connect(()=>{
        con.query(qry, [field], (error, result, field)=>{
